...
function renderResponseLine(line: string, demo: ApiDemoConfig): ReactNode {
  ...
  matches.forEach((match) => {
    ...
    if (start > cursor) {
      segments.push(
        <span key={`text:${cursor}-${start}`}>
          {tokenize(line.slice(cursor, start))}
        </span>
      )
    }
    ...
    if (placeholder === '<text>') {
      segments.push(
        <Accent key={`placeholder:${start}`} accent={demo.accent}>
          {`"${truncateResponse(demo)}"`}
        </Accent>
      )
    } else if (placeholder === '<tokens>') {
      segments.push(
        <NumberText key={`placeholder:${start}`}>{demo.tokens}</NumberText>
      )
    } else if (placeholder === '<in>') {
      segments.push(
        <NumberText key={`placeholder:${start}`}>
      ...
      segments.push(<Muted key={`placeholder:${start}`}>{placeholder}</Muted>)
    }
...
function tokenize(input: string): ReactNode {
  ...
  matches.forEach((match) => {
    ...
    if (isKey) {
      segments.push(<Key key={`key:${start}`}>{text}</Key>)
    } else {
      segments.push(<StringText key={`string:${start}`}>{text}</StringText>)
    }
