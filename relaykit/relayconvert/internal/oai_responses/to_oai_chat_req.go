	...
	case "reasoning":
		...
		for _, field := range []string{"content", "summary"} {
			...
			for _, rawPart := range parts {
				...
				if !ok {
					return nil, nil, reasoning.AsClientError(errors.New("reasoning text must be a string"))
				}
		...
		if !plaintextPresent {
			if encrypted, ok := item["encrypted_content"].(string); ok && encrypted != "" {
				return nil, nil, reasoning.AsClientError(errors.New("responses to chat conversion does not support encrypted-only reasoning"))
			}
