...
type QuotaData struct {
	...
	ClientTool string `json:"client_tool" gorm:"size:64;default:''"`
...
func SaveQuotaDataCache() {
	...
	for _, quotaData := range CacheQuotaData {
		...
		DB.Table("quota_data").
			Where("user_id = ? and username = ? and model_name = ? and created_at = ? and use_group = ? and token_id = ? and channel_id = ? and node_name = ? and COALESCE(client_tool, '') = ?",
				quotaData.UserID, quotaData.Username, quotaData.ModelName, quotaData.CreatedAt, quotaData.UseGroup, quotaData.TokenID, quotaData.ChannelID, quotaData.NodeName, quotaData.ClientTool).
			First(quotaDataDB)
		if quotaDataDB.Id > 0 {
...
// SourceQuotaData uses the same consumed-request totals as the model dashboard.
